...
class MeasurementService {
  ...
  /**
   * Close an in-flight timer without emitting a duration metric (failed/abandoned loads).
   */
  abandonTimeMeasurement(name: string): void {
    if (!this.timers.has(name)) {
      return;
    }

    this.timers.delete(name);
    rumEndTimeMeasure(sanitizeMeasurementName(name));
  }
